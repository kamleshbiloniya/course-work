...
int isGrater(NODE *num1 ,NODE *num2){
    num1=reverse(removeZero(num1));
    num2=reverse(removeZero(num2));
    int carry=1;
    ...
    if(num1 == NULL && num2 == NULL)return carry;
...
NODE *subtract(NODE *num1 , NODE * num2){
    ...
    if(num1 == NULL && num2 != NULL)return newNode(-1);
    return result;
...
NODE *divide(NODE *num1 ,NODE *num2){
    NODE * result=(NODE *)malloc(sizeof(NODE));
    result=NULL;
    result->data=0;
    NODE *one=newNode(1);
    //int s=size(num2);
    while(isGrater(num1,num2)){
...
int main(){
    ...
    for(int i=0; i<t; i++){
      ...
      switch(opcode){
         ...
         case 2: {
                        first=reverse(removeZero(reverse(first)));
                        second=reverse(removeZero(reverse(second)));
                     sub=removeZero(subtract(first,second));
                     printList(sub);
                     break;
         }
          case 3:{
              multiplication=multiply(first,second,n1,n2);
              printList(removeZero(multiplication));
              break;
              
          }
          case 4:{printf("%d",0);break;
              
              first=removeZero(reverse(first));
              second=removeZero(reverse(second));
              if(isGrater(second,first)){printf("%d",0);break;}
              division=divide(first,second);
              printList(division);break;
          }
          case 5:{      first=reverse(removeZero(reverse(first)));
                        second=reverse(removeZero(reverse(second)));
              printf("%d",isGrater(first,second));break;}
